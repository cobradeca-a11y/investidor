...
_ALIASES = {
    ...
    "data_base": ["data_base", "dt_base", "data_com", "data_comunicado", "Data Base", "Data COM"],
    "data_com": ["data_com", "dt_com", "data_base", "Data COM", "Data Base"],
...
def garantir_tabelas() -> None:
    ...
    # Garante campos mínimos na tabela operacional de dividendos.
...
def importar_arquivo(caminho_arquivo: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        if not colunas.get("valor") or not colunas.get("data_pagamento"):
            raise ValueError("Arquivo de dividendos FNET sem valor ou data de pagamento.")
        ...
        for _, row in df.iterrows():
            ...
            if not ticker or valor is None or not data_pagamento:
                ignorados += 1
                continue
        ...
        resumo = {"arquivo": str(caminho), "registros": total, "ignorados": ignorados, "tickers": tickers}
