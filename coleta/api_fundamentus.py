...
Correções aplicadas:
  - Encoding latin-1 forçado (evita parse errado e campos None em massa)
  - vacancia_media None tratado em coletar_mercado_inteiro (evita TypeError no radar)
"""
import re
...
def coletar_fii(ticker: str) -> Optional[dict]:
    ...
    try:
        ...
        res.encoding = 'latin-1'  # FIX: Fundamentus serve HTML em latin-1
...
def coletar_mercado_inteiro() -> List[dict]:
    ...
    try:
        ...
        res.raise_for_status()
        res.encoding = 'latin-1'  # FIX: encoding correto para evitar campos None em massa
        ...
        for row in corpo.find_all('tr'):
            ...
            vacancia_raw = _limpar_valor(cols[12])  # FIX: pode ser None para fundos de papel
            ...
            d = {
                ...
                "vacancia_media": vacancia_raw,  # None é válido — tratado no filtro
            }
            resultados.append(d)
