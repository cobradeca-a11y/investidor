...
from coleta.cotahist_b3 import garantir_tabela as garantir_tabela_cotahist
from coleta.cotahist_b3 import importar_zip_local as importar_cotahist_zip
...
def _registros_cotahist_existentes(ano: int) -> int:
    garantir_tabela_cotahist()
    row = db.buscar_um(
        "SELECT COUNT(*) AS total FROM cotacoes_b3 WHERE data >= ? AND data <= ?",
        (f"{ano}-01-01", f"{ano}-12-31"),
    )
    return int(row["total"] or 0) if row else 0


def importar_cotahist(downloads: Path, anos: list[int], force: bool = False) -> list[dict[str, Any]]:
    resultados: list[dict[str, Any]] = []
    for ano in anos:
        arquivo = _melhor_arquivo(downloads, f"COTAHIST_A{ano}.ZIP")
        if not arquivo:
            resultados.append({"ano": ano, "status": "AUSENTE", "tipo": "cotahist"})
            continue

        existentes = _registros_cotahist_existentes(ano)
        if existentes and not force:
            resultados.append(
                {
                    "ano": ano,
        ...
        resultado = importar_cotahist_zip(arquivo)
        resultado["ano"] = ano
        resultado["tipo"] = "cotahist"
        resultados.append(resultado)
    return resultados
...
def main() -> int:
    ...
    parser.add_argument("--importar-cotahist", action="store_true")
    ...
    if args.importar_cotahist:
        resumo["importacao_cotahist"] = importar_cotahist(downloads, args.anos, force=args.force)
