...
Objetivo:
- registrar decisões simuladas;
- registrar resultados observados;
- medir acerto, falso positivo e falso negativo;
- detectar deterioração de regra;
- ajustar pesos de forma gradual sem perder rastreabilidade.

Este módulo NÃO altera automaticamente o motor de decisão.
Ele gera evidências para ajuste controlado posterior.
"""
...
def registrar_simulacao(
    *,
    ...
    """Registra uma decisão simulada para avaliação futura."""
    garantir_tabelas()
    dados = {
...
def registrar_resultado(
    *,
    ...
    """Registra resultado de uma simulação e classifica falso positivo/negativo."""
    garantir_tabelas()
...
def resumo_aprendizado(janela_dias: int | None = None) -> dict[str, Any]:
    """Resume acertos, falsos positivos e falsos negativos."""
    garantir_tabelas()
    if janela_dias is None:
        rows = db.buscar_todos(f"SELECT * FROM {TABELA_RESULTADOS}")
    else:
...
def detectar_deterioracao_regra(min_amostras: int = 10, limite_falso_positivo: float = 0.35) -> list[dict[str, Any]]:
    """Detecta regras/condições com excesso de falso positivo."""
    garantir_tabelas()
    rows = db.buscar_todos(
        f"""
        SELECT s.gate55_status, s.fonte_patrimonial, r.falso_positivo, r.falso_negativo, r.resultado
    ...
    for row in rows:
        chave = f"gate55={row['gate55_status'] or 'N/D'}|fonte={row['fonte_patrimonial'] or 'N/D'}"
...
def sugerir_ajuste_peso(regra: str, peso_anterior: float, peso_sugerido: float, motivo: str, evidencia: str | None = None) -> dict[str, Any]:
    """Registra sugestão de ajuste sem aplicar automaticamente."""
    garantir_tabelas()
    dados = {
        "regra": regra,
